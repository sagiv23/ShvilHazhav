...
public class MemoryGameFragment extends BaseFragment implements MemoryGameAdapter.MemoryGameListener {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        recyclerCards = view.findViewById(R.id.recycler_OnlineMemoryGame);
        recyclerCards.setLayoutManager(new GridLayoutManager(getContext(), 3));

        tvTimer = view.findViewById(R.id.tv_OnlineMemoryGame_timer);
        tvTurnStatus = view.findViewById(R.id.tv_OnlineMemoryGame_turn_status);
        tvScore = view.findViewById(R.id.tv_OnlineMemoryGame_score);
        tvOpponentName = view.findViewById(R.id.tv_OnlineMemoryGame_opponent_name);

        adapter = adapterService.getMemoryGameAdapter();
        adapter.setListener(this);
        recyclerCards.setAdapter(adapter);

        view.findViewById(R.id.btn_OnlineMemoryGame_to_exit).setOnClickListener(v -> showExitGameDialog());
    ...
    private void showExitGameDialog() {
        Runnable onConfirm = () -> {
            ...
            navigateTo(R.id.action_memoryGameFragment_to_gameHomeScreenFragment);
        };
            ...
            databaseService.getImageService().getAllImages(new DatabaseCallback<>() {
                ...
                public void onCompleted(List<ImageData> allImages) {
                    if (allImages == null || allImages.size() < 6) {
                        Toast.makeText(requireContext(), "אין מספיק תמונות.", Toast.LENGTH_LONG).show();
